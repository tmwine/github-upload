/*
	...
	Monte Carlo routine for estimating Lyapunov exponent and variance, for alphabets of > 2 symbols
	this routine operates on a specified block row

	input:
	this expects, on the command line, <prod_len> <num_sam> <Mi*P data filename> <block row to use>
	<prod_len> is the number of matrices in each product chain
	<num_sam> is the number of product chains
	<Mi*P data file> is the name of the .bin file holding the relevant Mi*P matrix product set
	<block row to use> is the matrix block row to perform the Monte Carlo sampling on

	output:
	displays the Lyapunov exponent and variance in a standard format (readable for example by MultAlpha_Le_Var_MC)--"Lyapunov exponent...\n" and "Lyapunov variance...\n" each followed on the next line by the respective values
...
int main(int argc,char *argv[])
{
	...
	char *myfile = argv[3];
	long block_row_to_use = stol(argv[4]); // this should be an integer in [1,nbrows]
	...
	if (block_row_to_use>nbrows || block_row_to_use<1) {
		printf("\nProblem with requested block row; must be between 1 and %ld.", nbrows);
		return 1;
	}
	...
	if (!(MiP = new double*[k_sym])) {
		cout << "Error: out of memory." << endl;
		exit(1);
	}
	for (jj=0; jj<k_sym; jj++) {
		if (!(MiP[jj] = new double[mxsz_vals[block_row_to_use-1]*mxsz_vals[block_row_to_use-1]])) { // assign pointer to appropriate allocated spot in memory
			cout << "Error: out of memory." << endl;
			exit(1);
		}
	}
	...
	//***
	// read the desired block row from .bin file matrices
	//***

		// fseek to correct position (correct block row)
	for (ii=0; ii<block_row_to_use-1; ii++) {
		fseek(pfile,k_sym*mxsz_vals[ii]*mxsz_vals[ii]*sizeof(double),SEEK_CUR); // seeks past one block row at a time
	}

		// read block row matrices' entries from file
	long mx_sz = mxsz_vals[block_row_to_use-1];
	for (jj=0; jj<k_sym; jj++) {
		for (kk=0;kk<(mx_sz*mx_sz);kk++) {
	 		if (fread(&MiP[jj][kk], sizeof(double), 1, pfile) < 1) {
				printf("\nUnable to read from opened file.\n");
				return 1;
			}
		}
	...
	// DEBUG / display a matrix from the desired block row
	/*
	jj = 0; // for which symbol (indexed from 0)
	for (rr=0;rr<mx_sz;rr++) { // row number
		for (cc=0;cc<mx_sz;cc++) { // column number
			//cout << setprecision(14) << scientific << MiP[jj][mx_sz*rr+cc] << " ";
			cout << setprecision(4) << fixed << MiP[jj][mx_sz*rr+cc] << " ";
		}
		cout << endl;
	}
	cout << endl;
	*/
	...
	if (fabs(pv_cdf[k_sym-1]-1.0)>P_ERR) { // double check--the p_vals cdf vec should be very close to 1.0 in last position
